...
# çıktını tek satırda olmasını sağlar.
pd.set_option("display.expand_frame_repr",False)
from mlxtend.frequent_patterns import apriori, association_rules

# Adım 1: armut_data.csv dosyasınız okutunuz.
df_ = pd.read_csv("4.) Recommendation Systems/Projects (including Bonus)/ArmutARL-221114-234936/armut_data.csv")
df = df_.copy()
df.head()
df.info()
...
df["Serv_CatID"] = df["ServiceId"].astype(str) + "_" + df["CategoryId"].astype(str)
df.head()
df.info()
...
df["NewDate"] = pd.to_datetime(df["CreateDate"]).dt.to_period("M") ##***
...
basket_service_df = df.groupby(["BasketID","Serv_CatID"])["Serv_CatID"].count().unstack().fillna(0).applymap(lambda x: 1 if x>0 else 0)
df.info()

# Adım 2: Birliktelik kurallarını oluşturunuz.

frequent_itemsets = apriori(basket_service_df,
                            min_support=0.01,
                            use_colnames=True,
                            low_memory=True)
